			...
			if(isset($data['custom_style_temp']['tab-style'])){
				...
				foreach($data['custom_style_temp']['tab-style'] as $tabStyle=>$styleInfo){
					...
					if($tabStyle=='tab-style8'){
						...
						if($styleInfo['primary-color']!=''){
							$customStyle .= '.block'.$model->getId().' .mgs-product-tab .'.$tabStyle.'.data.items .item.title.active .switch,.block'.$model->getId().' .mgs-product-tab .'.$tabStyle.'.data.items .item.title .switch:hover{color: '.$styleInfo['primary-color'].' !important; border-color:'.$styleInfo['primary-color'].' !important}';
						}
					}
